...
- [x] **16-month backfill, production, 2026-07-20:** `curl` against `.../api/seo/sync/gsc/backfill` (no date params, defaults applied) — completed in a **single invocation**, 2m54s wall time, well inside the 300s Hobby ceiling (so `nextStartDate` chunking was never actually exercised against real data — see note below). **484 of 485 days** (2025-03-20 through 2026-07-17) processed successfully; one date, `2026-06-17`, recorded in `datesFailed` and skipped rather than aborting the run, exactly as `backfill.ts` is designed to behave. Retried that single date on its own — failed again, same result, with no error detail surfaced in the HTTP response (`backfill.ts`'s return shape only reports which dates failed, not why — a real gap, noted below). Root cause not yet determined; no Supabase MCP connector or direct log access was available this session to inspect `sync_log.metadata`/Vercel runtime logs for the actual error. Non-blocking — 484/485 days is a 99.8% success rate on the first-ever real backfill, and the single gap is a data-completeness note, not a pipeline defect (upsert-based re-sync means it can simply be retried once the cause is known).
- **Not yet performed:**
  - Real row values haven't been spot-checked against Search Console's own UI numbers for a given date
  ...
  - The actual cause of `2026-06-17`'s failure hasn't been diagnosed — needs either Supabase MCP access to read `sync_log.metadata` for that run, or Vercel function log access
...
- [x] 16-month backfill completed at least once — **484/485 days**, see above; the one gap is tracked as a follow-up, not treated as failing this criterion outright
- [ ] Confirmed against GSC's own UI totals — **not yet done, needs a manual spot-check in the Search Console UI**
- [ ] `ARCHITECTURE.md`'s "16-month retention" risk is substantially retired — **yes for 484 of the ~485 days now durably stored; the one gap should be closed once its cause is found**

**A real gap found while running this:** `runBackfillChunk`'s return shape (`datesProcessed`/`datesFailed`/`nextStartDate`) doesn't include *why* a date failed — the underlying error is only ever logged (`logger.error`) and written to that day's own `sync_log` row via `completeSyncRun`, neither of which was reachable from outside the app this session. A future improvement worth considering: include a `{date, error}` pair per failure in the response, not just the bare date — would have made this exact situation self-diagnosing via `curl` alone instead of needing DB/log access.
