...
- **Filtered to genuinely long-tail** (3+ words **and** real volume inside a floor/ceiling window — see "Post-ship corrections" below for why both bounds matter, not just word count) — re-checked in application code even though the API's own `filters` param already asks for the floor — same "don't trust a single filter blindly" discipline as every other DataForSEO response in this codebase.
...
**Post-ship corrections (both found from real production runs, same day):**
1. **Navigational intent excluded outright.** The first real run (8 seeds, 17 keywords, $0.10) surfaced 2 results — "empress restaurant birmingham", "...pershore road" — that turned out to be a competitor's actual business name, picked up because a generic niche seed happened to relate to it. Navigational intent means "searching for a specific known entity by name," which is structurally un-actionable here: you cannot rank content for someone else's brand search. Fixed in `extractCandidates()`; the 2 already-inserted rows were removed directly from production (confirmed zero referencing `actions` rows first).
2. **Word count alone doesn't define "long-tail" — added a real volume ceiling.** The user asked directly what made a result "long-tail" and pushed back on a word-count-only definition. Checked current SEO literature: the real, rigorous definition is a keyword's position on the search-demand curve (low individual volume, high specificity), not its word count, which is at best a loose correlate. Proven wrong by this module's own data: "jollof rice recipe" passed as long-tail purely for having 3 words, at 9,900 searches/month — the same order of magnitude as this site's actual broad, competitive terms ("mix grill" 9,900/mo, "suya" 8,100/mo, both real GSC-discovered keywords). Added `MAX_SEARCH_VOLUME = 5000`, calibrated from this site's own real data (genuinely niche dish/location-specific discoveries range 10-2,900/mo; 5,000 sits cleanly above that range and below the broad-term scale). 2 new tests; the already-inserted "jollof rice recipe" row was removed from production to match the corrected definition (confirmed it hadn't yet become an action).
