...
### Fixed
- **All 5 flagged cannibalization actions traced to one real conflict — the homepage and a blog post pitching the identical "nigerian restaurant in Handsworth/Birmingham" thing — and fixed with an actual content change, not a status flip.** Diagnosed via direct production queries (all 5 actions shared the exact same two `page_id`s) and by fetching both live pages' real content, which confirmed near-identical titles/pitches with zero differentiation. Rewrote the blog post in Sanity into a genuinely different, informational article ("Where Does Jollof Rice Really Come From?", new slug `jollof-rice-origin-story`) targeting a real long-tail keyword Milestone 15's own discovery run had found (320/mo, difficulty 10) instead of competing with the homepage's commercial intent — with a permanent redirect from the old slug. Caught a second, would-be overlap before shipping: the new angle duplicated ground an *existing* live post ("What Is Jollof Rice? A Beginner's Guide") already covered — trimmed that post's history section to a teaser with a real internal link instead of two pages quietly repeating each other. Removed the now-stale hardcoded fallback for the old blog post from `src/sanity/fallbacks.ts`. All 5 actions marked `completed` with real `baseline_metrics` captured, so Milestone 13's outcome tracking reports back in ~30 days on whether it worked. Also found and dismissed 3 separate `create_content` actions that would have recreated the same cannibalization by targeting the same/adjacent keywords as new dedicated pages.
- **Opportunity Score had no volume ceiling, so it was recommending content for keywords no single-location restaurant could ever rank for** — "restaurants near me" (9.14M searches/month) and "restaurant" (7.48M/mo) both surfaced as top-priority "opportunities" in the same queue as genuine finds like "mix grill" (9,900/mo, a real menu item). Added `MAX_ACTIONABLE_VOLUME = 50,000` to `run-analysis.ts`'s Opportunity Score → `create_content` candidate step (not the frozen ARCHITECTURE.md §5.1 scoring formula itself) — calibrated to exclude the actually-absurd multi-million-scale head terms while keeping real menu-item-scale opportunities. Documented as a real, partial fix: some borderline generic "near me" terms still clear the ceiling. 1 new regression test (477 total). See PHASE_2_IMPLEMENTATION.md Milestone 17.
