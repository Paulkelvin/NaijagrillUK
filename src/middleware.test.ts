...
describe("middleware", () => {
  it("matcher covers /admin and /api/seo/status (ARCHITECTURE.md §7)", () => {
    expect(config.matcher).toEqual(expect.arrayContaining(["/admin", "/admin/:path*", "/api/seo/status"]));
  });

  for (const path of ["/admin", "/api/seo/status"]) {
    describe(`path: ${path}`, () => {
      beforeEach(() => {
